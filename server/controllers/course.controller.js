...
// In-memory cache to track active student watch sessions: courseId -> { userId -> timestamp }
export const activeWatchingStudents = {};
...
export const getCreatorCourses = async (req, res) => {
  try {
    ...
    // Map each course and inject liveWatching count
    const coursesWithLiveCount = courses.map(course => {
      const courseObj = course.toObject();
      const active = activeWatchingStudents[course._id.toString()] || {};
      const now = Date.now();
      
      let liveCount = 0;
      for (const id in active) {
        if (now - active[id] <= 20000) {
          liveCount++;
        } else {
          delete active[id];
        }
      ...
      courseObj.liveWatching = liveCount;
      return courseObj;
    });

    return res.status(200).json({
      success: true,
      courses: coursesWithLiveCount,
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
...
export const updateHeartbeat = async (req, res) => {
  try {
    ...
    if (!activeWatchingStudents[courseId]) {
      activeWatchingStudents[courseId] = {};
    }

    activeWatchingStudents[courseId][userId] = Date.now();

    // Prune stale viewers (older than 20 seconds) for this course
    const now = Date.now();
    const active = activeWatchingStudents[courseId];
    for (const id in active) {
      if (now - active[id] > 20000) {
        delete active[id];
      }
    ...
    return res.status(200).json({
      ...
      liveCount: Object.keys(active).length,
    });
  } catch (error) {
