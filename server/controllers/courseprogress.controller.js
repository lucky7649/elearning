...
export const submitQuiz = async (req, res) => {
  try {
    ...
    // Check all answers
    const allCorrect = lecture.quiz.every(
      (q, i) => Number(answers[i]) === q.correctAnswerIndex
    );

    if (!allCorrect) {
      // Find which ones are wrong
      const wrongCount = lecture.quiz.filter(
        (q, i) => Number(answers[i]) !== q.correctAnswerIndex
      ).length;
      return res.status(200).json({
        passed: false,
        message: `${wrongCount} answer(s) incorrect. Please reattempt.`,
      });
    }

    // All correct — update progress
    let courseProgress = await CourseProgress.findOne({ courseId, userId });
    if (!courseProgress) {
      courseProgress = new CourseProgress({
        userId,
    ...
    const idx = courseProgress.lecturesProgress.findIndex(
      (l) => l.lectureId === lectureId
    );
    if (idx !== -1) {
      courseProgress.lecturesProgress[idx].quizPassed = true;
    } else {
      courseProgress.lecturesProgress.push({
        lectureId,
        viewed: true,
    ...
    await courseProgress.save();
    return res.status(200).json({ passed: true, message: "Quiz passed! Next lecture unlocked." });
  } catch (error) {
    console.error(error);
